import {
	...
	type TableInclusionType,
	type TablePaymentTiming,
} from "@prisma/client";
import {
	Armchair,
	Ban,
...
import { BlackoutDatesEditor } from "@/components/admin/BlackoutDatesEditor";
...
function _enumOptions<T extends Record<string, string>>(values: T) {
...
function buildTablesHref(
	...
	return `/dashboard/${slug}/reservations/tables${query ? `?${query}` : ""}`;
}
...
export default async function TablesPage({
	...
	const restaurant = await db.restaurant.findFirstOrThrow({
		...
		select: {
			...
			blackoutDates: { select: { date: true, reason: true } },
			tables: {
	...
	// Restaurant level is policy only now — the booking rules live on each table.
	const setting = restaurant.reservationSetting ?? {
		advanceBookingHours: 0,
		slotIntervalMinutes: 30,
		graceMinutes: 15,
		autoConfirmFreeBookings: false,
		bookingDescription: "",
		cancellationPolicy: "",
		refundPolicy: "",
	};
	const tableRows = restaurant.tables.map((table) => {
		...
		const displayMinimumSpend = table.minimumSpend ?? table.tableFee;
			...
			<div className="hidden gap-4 md:grid xl:grid-cols-[minmax(0,1fr)_auto] xl:items-end">
				...
				<div className="flex flex-wrap gap-3">
					<SettingsPanel
						restaurantSlug={restaurant.slug}
						slug={slug}
						blackoutDates={restaurant.blackoutDates}
						setting={setting}
...
function SettingsPanel({
	...
	setting: {
		...
		graceMinutes: number;
		autoConfirmFreeBookings: boolean;
	...
	return (
		<details className="group relative">
			...
			>
				...
				{/* Booking rules moved onto each table — a window seat for two and a
				    private room for twelve never shared a hold time or a minimum
				    spend. What is left here is genuinely one-per-restaurant. */}
				<div className="grid gap-3 md:grid-cols-3">
					<label className={labelClass}>
						<FieldLabel help={helpText.advanceHours}>
					...
					<label className={labelClass}>
						...
						<select
							...
							defaultValue={String(setting.slotIntervalMinutes)}
							className={inputClass}
						>
						...
						<span className="text-xs font-medium text-slate-400">
							Customers pick from these times instead of typing any minute.
						</span>
					</label>
					<label className={labelClass}>
						<span className="text-xs font-black text-slate-700">
							Hold a late guest for
						</span>
						<input
							name="graceMinutes"
							type="number"
							min="0"
							max="120"
							defaultValue={setting.graceMinutes}
							className={inputClass}
						/>
						<span className="text-xs font-medium text-slate-400">
							Minutes past the booking time before the table is released.
						</span>
					</label>
				</div>

				<label className="flex min-w-0 cursor-pointer items-start gap-3 rounded-xl border border-slate-200 p-3">
					<input
						type="checkbox"
						name="autoConfirmFreeBookings"
						defaultChecked={setting.autoConfirmFreeBookings}
						className="mt-0.5 size-4 shrink-0 rounded border-slate-300 text-emerald-600 focus:ring-emerald-500"
					/>
					<span className="min-w-0">
						<span className="block text-sm font-black text-slate-900">
							Confirm free bookings automatically
						</span>
						<span className="block text-xs font-medium leading-5 text-slate-500">
							Only applies when nothing is owed and the table has been checked
							as free for that slot. Bookings that take a deposit or payment
							always wait for you.
						</span>
					</span>
				</label>

				{/* Sits outside the settings form on purpose: closures are added one
				    at a time and take effect immediately, rather than waiting for a
				    Save that also rewrites unrelated policy text. */}
				<div className="rounded-2xl border border-slate-200 p-4">
					<BlackoutDatesEditor slug={slug} dates={blackoutDates} />
				</div>
