...
export async function submitRatingAction(formData: FormData) {
	return actionResult(async () => {
		...
		if (order.rating) {
			throw new ActionError("This order has already been rated.");
		}

		if (!isOrderRatingEligible(order)) {
			throw new ActionError("This order is not ready to be rated yet.");
		}
		...
		await db.rating.create({
			data: {
				...
				serviceQuality: applicableMetrics.has("serviceQuality")
					...
					: undefined,
				ambiance: applicableMetrics.has("ambiance")
					? input.ambiance
					: undefined,
